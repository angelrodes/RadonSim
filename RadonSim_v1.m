...
%% Select files
radon_data_file='2930129618-latest(41).csv';
ventilation_file='ventilation_Office217.csv';
...
unique_days_ventilated=unique(floor(input.numeric_time/1e6)*1e6);
unique_days=...
    unique_days_input(...
...
n_3s_models=200;
n_1s_models=100;
n_models=n_random_models+n_3s_models+n_1s_models;
...
minimum_Rn=10;
maximum_Rn=max(model.instant_Rn);
...
for n=1:n_models
    ...
    if n>n_random_models
        if n<n_random_models+n_3s_models
            if sum(model.red_chi_square<min(model.red_chi_square)+3)>24
                parameter_limits=[...
                    min(model.parameters(model.red_chi_square<min(model.red_chi_square)+3,:)) ; ...
                    max(model.parameters(model.red_chi_square<min(model.red_chi_square)+3,:)) ...
                    ];
            else
                sorted_red_chi_square=sort(model.red_chi_square);
                select=model.red_chi_square<=sorted_red_chi_square(24);
                parameter_limits=[...
            ...
            if sum(model.red_chi_square<min(model.red_chi_square)+1)>8
                parameter_limits=[...
                    min(model.parameters(model.red_chi_square<min(model.red_chi_square)+1,:)) ; ...
                    max(model.parameters(model.red_chi_square<min(model.red_chi_square)+1,:)) ...
                    ];
            else
                sorted_red_chi_square=sort(model.red_chi_square);
...
ventilation_time=(onesigma_params(:,2)-onesigma_params(:,1))./onesigma_params(:,3);
  disp(['    Effective ventilation time needed to flush Rn: ' num2str(round(min(ventilation_time)/60/60)) ' to ' num2str(round(max(ventilation_time)/60/60)) ' hours'])
accumulation_time=(300-onesigma_params(:,1))./onesigma_params(:,4);
if best_params(1)<300
  disp(['    Maximum accumulation time with safe Rn levels: ' num2str(round(min(accumulation_time)/60/60)) ' to ' num2str(round(max(accumulation_time)/60/60)) ' hours'])
else
...
plot(input.posix_time,input.posix_time.*0+300,'--r','LineWidth',2)
text(model.posix_time(1),300,'Maximum safe concentration',...
    'VerticalAlignment','Bottom','Color','r')

% plot input data
sel=~isnan(input.Rn);
plot(input.posix_time(sel),input.Rn(sel),'-k','LineWidth',2)
sel=find(input.Rn==max(input.Rn),1,'first');
text(input.posix_time(sel),input.Rn(sel),'Data (24h av.)',...
    'VerticalAlignment','Bottom','Color','k')

% plot model
% plot(model.posix_time,model.instant_averaged_24h,'.','Color','g')
plot(model.posix_time,bestmodel_concentrations,'-b','LineWidth',2)
plot(input.posix_time,bestmodel_24h_average,':b')
sel=find(bestmodel_concentrations==max(bestmodel_concentrations),1,'first');
text(model.posix_time(sel),bestmodel_concentrations(sel),'Model (instant and 24h av.)',...
...
plot(model.posix_time(sel),model.ventilated(sel)*max_y_plot*1.1,'.g')
text(min(model.posix_time(sel)),max_y_plot*1.1,'Ventilated',...
